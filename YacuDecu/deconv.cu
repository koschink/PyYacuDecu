...
__global__ void FloatDiv(float *A, float *B, float *C)
{
    unsigned int i = blockIdx.x * gridDim.y * gridDim.z * blockDim.x + blockIdx.y * gridDim.z * blockDim.x + blockIdx.z * blockDim.x + threadIdx.x;
    if (B[i] > 0.00001){ 
    C[i] = A[i] / B[i]; //avoid div0 errors, e.g. if the PSF or image was 0 padded; if the data is >0, everything is processed normally, otherwise it will be set to 0
	}
	else {C[i] = 0;}
}
...
int deconv_device(unsigned int iter, size_t N1, size_t N2, size_t N3, 
    ...
    for(unsigned int i=0; i < iter; i++) {
        printf("Iteration %d\n", i);
        r = cufftExecR2C(planR2C, object, (cufftComplex*)buf); // FFT of the object, stored in buffer
        if(r) goto cufftError;
        ComplexMul<<<freqBlocks, freqThreadsPerBlock>>>((cuComplex*)buf, otf, (cuComplex*)buf); // multiplication the obejct stored in buffer with the OTF in fourier space
        r = cufftExecC2R(planC2R, (cufftComplex*)buf, (float*)buf); //inverse FFT of the buffer, in place ==> object is in the spatial domain again
        if(r) goto cufftError;
        FloatDiv<<<spatialBlocks, spatialThreadsPerBlock>>>(image, (float*)buf, (float*)buf); // division (in the spatial domain) of the original image with the buffer (ratio), stored in buffer 
        r = cufftExecR2C(planR2C, (float*)buf, (cufftComplex*)buf); //transform the ratio back to frequency space
        if(r) goto cufftError;
        ComplexMul<<<freqBlocks, freqThreadsPerBlock>>>((cuComplex*)buf, otf, (cuComplex*)buf); // multipy the ratio with the OTF
        r = cufftExecC2R(planC2R, (cufftComplex*)buf, (float*)buf); // transform 
        if(r) goto cufftError;
